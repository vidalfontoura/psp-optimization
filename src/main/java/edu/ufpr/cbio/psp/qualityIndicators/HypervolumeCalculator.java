    ...
    public double execute(SolutionSet front) {
        ...
        if (internalPopulation.size() != 0) {
            double[] maximumValues =
                metricsUtil.getMaximumValues(internalPopulation.writeObjectivesToMatrix(), numberOfObjectives);
            for (int i = 0; i < maximumValues.length; i++) {
                // Adding an offset to properly calculate the hypervolume
                // reference point
                maximumValues[i] = maximumValues[i] + offset;
            }
            double[] minimumValues =
                metricsUtil.getMinimumValues(internalPopulation.writeObjectivesToMatrix(), numberOfObjectives);
            // return hypervolume.hypervolume(front.writeObjectivesToMatrix(),
            // maximumValues, minimumValues);
            return hypervolume.hypervolume(front.writeObjectivesToMatrix(),
                internalPopulation.writeObjectivesToMatrix());
        }
        return 0D;
    }

...
public class NSGAIITuningMultiobjectiveMain {
	...
	public static void main(String[] args) throws Exception {
		...
		File proteinChainDir = createDir(
				rootDir.getPath() + File.separator + ProteinChainUtils.getNotationByProteinChain(proteinChain));
						...
						for (int u = 0; u < crossoverProbabilities.length; u++) {
							...
							for (int t = 0; t < mutations.length; t++) {
								for (int h = 0; h < mutationProbabilities.length; h++) {
									...
									// configuration
									NSGAIIExecutingTask nsgaiiExecutingTask = new NSGAIIExecutingTask(crossover,
											Double.valueOf(crossoverProbability), mutation,
											Double.valueOf(mutationProbability), Integer.valueOf(population),
	...
	private static Crossover getCrossover(String crossoverName, double crossoverProbability) {
		...
		case "IntegerTwoPointsCrossover": {
			crossover = new IntegerTwoPointsCrossover.Builder().setCrossoverProbability(crossoverProbability).build();
			break;
		}
